...
export default async function azureFunctionMiddleware(
  ...
  const [core, hostJSON] = await Promise.all([
    getCore(),
    files
      .readFile("host.json")
...
function addMissingAttributes(hostJSON: any): string {
  ...
  return createObjectWithPropertiesSorted(hostJSON, ["version"]);
}
