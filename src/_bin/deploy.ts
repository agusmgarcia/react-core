...
async function validatePositionOfTheTag(
  typeOfNewVersion: ReturnType<typeof findTypeOfNewVersion>,
...
async function checkoutDefaultAndDeleteCurrent(): Promise<void> {
...
async function createNextRelease(
  ...
  await git.createBranch(
    `releases/v${nextTagInfo.major}.${typeOfNewVersion === "minor" ? nextTagInfo.minor + 1 : nextTagInfo.minor}.${typeOfNewVersion === "minor" ? 0 : nextTagInfo.patch + 1}`,
    nextTag,
  );

  try {
    await git.cherryPick(lastTagMerged, newTag);
  } catch (error) {
    console.error(error);
    return;
  }
